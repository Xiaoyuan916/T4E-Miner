...
Weak-label construction: experimentally characterized T4Es are used as seed enzymes for structure-guided homolog retrieval, followed by catalytic-site conservation filtering.

Model fine-tuning: the curated positive and negative datasets are used to fine-tune the terminal layers of ESM-C 300M together with a binary classification head.

Metagenomic inference: the fine-tuned model predicts and scores candidate T4Es from metagenomic protein sequences.

Candidate stratification: predicted candidates are grouped according to their sequence identity to the experimentally characterized T4E A0A662GG58 to guide representative selection for experimental validation.
...
Zenodo: processed datasets and trained checkpoint
